...
module.exports = (db) => {
    ...
    app.post('/rides', jsonParser, async (req, res) => {
        try {
            ...
            const rows = await db.allPromise('SELECT * FROM Rides ORDER BY rideID DESC LIMIT 1')
    ...
    app.get('/rides', async (req, res) => {
        try {
            ...
            const rows = await db.allPromise(`SELECT * FROM Rides ${pagination.pageClause}`, pagination.args)
            if (rows.length === 0) {
                return res.send({
    ...
    app.get('/rides/:id', async (req, res) => {
        try {
            const rows = await db.allPromise('SELECT * FROM Rides WHERE rideID = ?', req.params.id)
            if (rows.length === 0) {
                return res.send({
                    error_code: 'RIDES_NOT_FOUND_ERROR',
                    message: 'Could not find any rides'
